...
For Cumulocity-specific input and output blocks:
...
Extra functionalities are provided by API for input and output blocks consuming or producing to Cumulocity devices:
...
Note: For migrating input blocks that were using the Cumulocity helper API, see the section **Migrating custom Cumulocity input blocks**.
...
Note: For migrating output blocks that were using the  Cumulocity helper API, see the section **Migrating custom Cumulocity output blocks**.
...
```
## Migrating custom Cumulocity blocks

The following section is specific to blocks that input or output data associated with a Cumulocity device or device group. To reduce the code duplication across Cumulocity input/output blocks, a set of helper events is provided. See [Cumulocity-specific helpers](105-CumulocityHelper.md) for more details.

### Migrating custom Cumulocity input blocks

The handler API provides the  `CumulocityInputHandler` event to deal with Cumulocity input blocks. 
...
Using the version 1 API, scheduling input for a custom Cumulocity block is as follows:
```java
/**
 * Method starts listening for alarms from Cumulocity.
 */
action $init() {
	string id;
...
```
Using the new handler API, scheduling an input event is done by calling the `schedule` action on the Cumulocity handler object. The handler will determine the correct partition value and will check for you if the event should be dropped. Events dropped by the framework when `schedule` is called do not need to be reported using the `droppedEvent` action.
```java
/**
 * Method starts listening for alarms from Cumulocity.
 */
...
```
### Migrating custom Cumulocity output blocks

The handler API provides the  `CumulocityOutputHandler` event to deal with Cumulocity blocks.
...
[< Prev: Asynchronous validations](110-AsynchronousValidations.md) | [Contents](000-contents.md) | [Next: Update Cumulocity input blocks to receive from assets >](151-MigrateInputBlocksForAssetInput.md) 
